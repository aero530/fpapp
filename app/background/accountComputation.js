...
export default function accountComputation(accounts, settings) {
  ...
  const incomeTotalTable = [...emptyYearTable];
  const incomeTotalAfterTaxTable = [...emptyYearTable];
  const netTable = [...emptyYearTable];
  ...
  Object.entries(accounts).forEach(([accountName, account]) => {
    ...
    // # ----------------------------------------------------------------------
    let tmp = account.table[0];
    account.table = [...emptyYearTable]; // make array the same length as yearDelta populated with zeros
    account.table[0] = tmp;
    ...
    // # ----------------------------------------------------------------------
    if (
      Object.prototype.hasOwnProperty.call(account, 'startin') &&
      typeof account.startin !== 'number'
    ) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(account, 'endin') &&
      typeof account.endin !== 'number'
    ) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(account, 'startout') &&
      typeof account.startout !== 'number'
    ) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(account, 'endout') &&
      typeof account.endout !== 'number'
    ) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(account, 'raise') &&
      typeof account.raise !== 'number'
    ) {
  ...
  yearDelta.forEach(yearIndex => {
    ...
    // # ----------------------------------------------------------------------
    if (yearIndex > 0) {
      netTable[yearIndex] = netTable[yearIndex - 1]; // initialize this year as the value from last year
    }
    ...
    accountOrderIndex.forEach(accountName => {
      ...
      if (account.type === 'expense' || account.type === 'income') {
        ...
        account.table[yearIndex] = 0; // previous years value does not carry over (ie not an account that carries a balance)
      } else {
        // this account type should carry over the value from last year
        if (yearIndex > 0) {
          account.table[yearIndex] = account.table[yearIndex - 1];
        }
      }
      ...
      if (
        ...
        earnings = (account.table[yearIndex] * account.yearlyreturn) / 100; // calculate earnings from interest
        account.earnings[yearIndex] = earnings; // set account earnings to current earnings value
      } else if (account.type === 'income') {
        // Otherwise if this is an INCOME account (here ear)
        if (account.startin <= yearCurrent && account.endin >= yearCurrent) {
          // if this income object is active this year
          earnings =
            account.base *
            Math.pow(1 + account.raise / 100, yearCurrent - account.startin); // calculate this years income
        }
      }
      ...
      if (account.type === 'loan') {
        ...
        if (
          ...
          interest =
            account.table[yearIndex] *
              Math.pow(
                1 + account.rate / 100.0 / account.compoundtime,
                account.compoundtime
              ) +
            account.mortgageinsurance +
            account.escrowvalue -
            account.table[yearIndex]; // add this years interest to the mortgage then decrease mortgage by payment amount but reduce payment by the escrow and mortgage insurance values
        } else {
          ...
          interest =
            ...
            account.escrowvalue -
            account.table[yearIndex]; // add this years interest to the mortgage athe make payment on mortgage but but reduce payment by escrow value
          account.interest[yearIndex] = interest;
        ...
        if (account.startin <= yearCurrent && account.endin >= yearCurrent) {
          ...
          if (account.contributiontype === 'fixed_with_inflation') {
            ...
            contribution =
              ...
              Math.pow(1 + inflationBase / 100, yearDelta[yearIndex]); // increase the value by inflation
            ...
            if (
              ...
              account.incomelink.length > 0
            ) {
              // and if the account has an income object linked to it
              contribution =
                accounts[account.incomelink].table[yearIndex] *
                (account.yearlycontribution / 100); // calculate the contribution value using that income account and the percentage input
            } else {
              // otherwise
              contribution =
                incomeTotalTaxableTable[yearIndex] *
                (account.yearlycontribution / 100); // calculate the contribution using the total income for the year
            }
          }
          ...
          if (
            ...
            if (account.incomelink.length >= 0) {
              ...
              if (account.employermatch[0] >= 0 && account.matchlimit[0] >= 0) {
                if (account.matchlimit.length > 1) {
                  ...
                  if (
                    contribution >=
                    (account.matchlimit[0] / 100 +
                      account.matchlimit[1] / 100) *
                      accounts[account.incomelink].table[yearIndex]
                  ) {
                    // and if the contribution is above the highest employer matching level
                    employermatch =
                      accounts[account.incomelink].table[yearIndex] *
                      ((account.employermatch[1] / 100) *
                        (account.matchlimit[1] / 100) +
                        (account.employermatch[0] / 100) *
                          (account.matchlimit[0] / 100)); // calculate the employer matching based on the match limits
                  } else if (
                    contribution >=
                    (account.matchlimit[0] / 100) *
                      accounts[account.incomelink].table[yearIndex]
                    ...
                    employermatch =
                      ...
                      ((account.employermatch[0] / 100) *
                        ...
                        (account.employermatch[1] / 100) *
                          ...
                          (contribution /
                            accounts[account.incomelink].table[yearindex] -
                            account.matchlimit[0] / 100)); // calculate the employer matching with all the first level and part of the second level
                  } else {
                    employermatch =
                      contribution * (account.employermatch[0] / 100); // the employer contribution is computed based on the entire contribution
                  }
                } else {
                  // if it is a simple employer matching (only one level)
                  if (
                    contribution >=
                    account.matchlimit[0] *
                      accounts[account.incomelink].table[yearIndex]
                  ) {
                    // and if the contribution is above the highest employer matching level
                    employermatch =
            ...
            yearCurrent <= yearRetire
          ) {
            if (account.contributiontype === 'fixed_with_inflation') {
              // if inflation needs to be accounted for in the contribution
              employermatch =
                account.employercontribution *
                Math.pow(1 + inflationBase / 100, yearDelta[yearIndex]); // increase the value by inflation
            } else if (account.contributiontype === 'fixed') {
              // otherwise if the contribution is a fixed value
      ...
      if (Object.prototype.hasOwnProperty.call(account, 'paymenttype')) {
        ...
        if (account.startout <= yearCurrent && account.endout >= yearCurrent) {
          ...
          if (payment > account.table[yearIndex]) {
            payment = account.table[yearIndex];
          }
      ...
      if (Object.prototype.hasOwnProperty.call(account, 'withdrawaltype')) {
        if (account.startout <= yearCurrent && account.endout >= yearCurrent) {
          ...
          if (account.withdrawaltype === 'col_frac_of_savings') {
            ...
            if (yearIndex > 1) {
              ...
              if (account.table[yearIndex - 1] > 0) {
                ...
                let totalExpensesThisYear = Object.values(
                  expenseTotal[yearIndex]
                ).reduce((acc, cur) => acc + cur, 0);
                withdrawal =
                  (totalExpensesThisYear * account.table[yearIndex - 1]) /
                  savingsTotalTable[yearIndex - 1];
                if (
                  Object.prototype.hasOwnProperty.call(account, 'taxstatus') &&
                  account.taxstatus === 3
                ) {
                  withdrawal = withdrawal * (taxIncome / 100 + 1); // add extra to amount withdrawal value to account for taxes.
                }
            ...
            // otherwise if type is a fixed number but should be compensated for with inflation
            withdrawal =
              account.withdrawalvalue *
              Math.pow(1 + inflationBase / 100, yearDelta[yearIndex]); // set withdrawal to the value multiplied by an increase due to inflation
            ...
            if (account.endout >= yearCurrent) {
              ...
              withdrawal =
                account.table[yearIndex] / (account.endout - yearCurrent + 1); // calculate the fraction of the account balance to withdraw
            }
          } else {
            // otherwise if a different type is specified
        ...
        if (withdrawal > account.table[yearIndex]) {
          // dont allow an account to become overdrawn
          withdrawal = account.table[yearIndex];
        }
        if (withdrawal < 0) {
      ...
      if (account.type === 'expense') {
        ...
        if (account.startout <= yearCurrent && account.endout >= yearCurrent) {
          ...
          if (account.expensetype === 'fixed') {
            ...
            // otherwise if type is a fixed number but should be compensated for with inflation
            expense =
              account.expensevalue *
              Math.pow(1 + inflationBase / 100, yearDelta[yearIndex]); // set expense to the value multiplied by an increase due to inflation
          } else {
            // otherwise if a different type is specified
      ...
      // # ----------------------------------------------------------------------
      account.table[yearIndex] += earnings;
      ...
      if (account.type === 'income') {
        incomeTotalTaxableTable[yearIndex] =
          incomeTotalTaxableTable[yearIndex] + account.table[yearIndex]; // increase this years taxable income by the withdrawal amount
        incomeTotalTable[yearIndex] =
          incomeTotalTable[yearIndex] + account.table[yearIndex]; // increase this years income by the withdrawal amount
      }
      ...
      // # ----------------------------------------------------------------------
      account.table[yearIndex] = account.table[yearIndex] + interest;

      // # ----------------------------------------------------------------------
      // # Add contribution and employermatch to the account for the year
      // # ----------------------------------------------------------------------
      account.table[yearIndex] =
        account.table[yearIndex] + contribution + employermatch;
      if (
        Object.prototype.hasOwnProperty.call(account, 'taxstatus') &&
        ...
        incomeTotalTaxableTable[yearIndex] =
          incomeTotalTaxableTable[yearIndex] - contribution; // take the contribution value out of taxable income for the year
      }

      // # ----------------------------------------------------------------------
      // # Add payment to the account for the year
      // # ----------------------------------------------------------------------
      account.table[yearIndex] = account.table[yearIndex] - payment;

      // # ----------------------------------------------------------------------
      // # Add withdrawal to the account for the year
      // # ----------------------------------------------------------------------
      account.table[yearIndex] = account.table[yearIndex] - withdrawal;
      if (account.type !== 'college') {
        ...
        if (
          ...
          incomeTotalTaxableTable[yearIndex] =
            incomeTotalTaxableTable[yearIndex] + withdrawal; // increase this years taxable income by the withdrawal amount
          incomeTotalTable[yearIndex] =
            incomeTotalTable[yearIndex] + withdrawal; // increase this years taxable income by the withdrawal amount
        } else {
          incomeTotalTable[yearIndex] =
            incomeTotalTable[yearIndex] + withdrawal; // increase this years taxable income by the withdrawal amount
        }
      ...
      if (
        ...
        incomeTotalTaxableTable[yearIndex] -= expense; // take the expense value out of taxable income for the year
      }

      account.table[yearIndex] += expense;
      ...
      if (
        ...
        if (Object.prototype.hasOwnProperty.call(account, 'hsalink')) {
          ...
          if (accounts[account.hsalink].table[yearIndex] >= expense) {
            // if there is enough money in the HSA savings account to pay for healthcare expenses
            accounts[account.hsalink].table[yearIndex] -= expense;
            ...
            accounts[account.hsalink].withdrawal[yearIndex] =
              accounts[account.hsalink].table[yearIndex];
            let tmp = expense - accounts[account.hsalink].table[yearIndex];
            accounts[account.hsalink].table[yearIndex] = 0;
            expense = tmp;
          }
        }
      }
      ...
      if (account.type === 'expense') {
        ...
        expenseTotal[yearIndex][accountName] = expense; // add withdrawal to the expense table
      } else if (account.type === 'loan') {
        // otherwise if type is LOAN
        expenseTotal[yearIndex][accountName] = payment; // add withdrawal to the expense table
      } else if (account.type === 'mortgage') {
        // otherwise if type is LOAN
        expenseTotal[yearIndex][accountName] = payment; // add withdrawal to the expense table
      } else if (account.type === 'college') {
        // otherwise if type is LOAN
        expenseTotal[yearIndex][accountName] = contribution; // add contribution to the expense table
      ...
      if (account.type === 'savings' || account.type === 'retirement') {
        ...
        savingsTotalTable[yearIndex] += account.table[yearIndex];
      }
    });
    ...
    // # ----------------------------------------------------------------------
    const totalExpensesThisYear = Object.values(expenseTotal[yearIndex]).reduce(
      (acc, cur) => acc + cur,
      0
    );
    netTable[yearIndex] =
      netTable[yearIndex] +
      incomeTotalTable[yearIndex] -
      incomeTotalTaxableTable[yearIndex] * (taxIncome / 100) -
      totalExpensesThisYear;
    incomeTotalAfterTaxTable[yearIndex] =
      incomeTotalTable[yearIndex] -
      incomeTotalTaxableTable[yearIndex] * (taxIncome / 100);
